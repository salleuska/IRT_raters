...
code2PL2PL <- nimbleCode({
  ...
  for(n in 1:tot){
    ...
    for(k in 1:K){
      pic[n,k]          <-  exp(  (lambda[II[n]] + phi[RRi[n]]) * (eta[PPi[n]] + beta[II[n]] + delta[k] + tau[RRi[n]] ) )   # Adjacent categories logits (Agresti, 2013, pp.309-310; Masters, 1982, p.158)
  ...
  for(i in 1:(R-1)) {
    ...
    phi[i] ~ dlnorm(meanlog = 0, sdlog = 3)
  }
  
  tau[R]    <-  -sum(tau[1:(R-1)])
  phi[R] <-  -sum(log(phi[1:(R-1)]))
...
inits <- list(beta   = rnorm(constants$I, 0, 3),
              ...
              phi    = rep(1, constants$R)
)

monitors = c("beta","delta", "lambda","tau","phi", "eta")
...
tauCols <- grep("phi", colnames(samples))
...
par(mfrow = c(2, 2), cex = 1.1)
for(i in 1:R)
  ts.plot(samples[ , tauCols[i]], xlab = 'iteration', ylab = colnames(samples)[ phiCols[i]])
