...
generateData <- function( mu, sigma, c_p_s_s, N, R, tot, PPi, II, RRi, ARi, lambda, beta, delta, R_tau, R_phi) {
  ...
  for ( n in 1:tot ) {
    ...
    if(ARi[n]==0){
      ...
      for(k in 2:K){
        pic[n,k]          <-  exp(R_phi[RRi[n]] + lambda[II[n]]) * (eta[PPi[n]] - beta[II[n]] + delta[k-1] - R_tau[RRi[n],1]  )   # Adjacent categories logits (Agresti, 2013, pp.309-310; Masters, 1982, p.158)
...
write.csv(Data, file="Data_2PL2PL.csv")
